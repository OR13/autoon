/**
 * Autoon Converter - JSON to TOON and back
 */

import type { AutoonDocument, Graph, Node, Edge } from './types.js';
...
export function toonToJson(toon: string): AutoonDocument {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Parse data lines (indented with 2 spaces)
    if (line.startsWith('  ')) {
      ...
      if (currentSection === 'nodes') {
        ...
        fields.forEach((field, idx) => {
          ...
          if (val) {
            ...
            if (field === 'type') node.type = val as Node['type'];
          }
        });
      ...
      if (currentSection === 'edges') {
        ...
        fields.forEach((field, idx) => {
          ...
          if (val) {
            ...
            if (field === 'relation') edge.relation = val as Edge['relation'];
